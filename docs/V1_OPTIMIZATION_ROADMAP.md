...
- ✅ Phase D 步骤 2（P-003-2 UI）：`/radar/today` 侧栏新增"生成今日核心报告"
  **POST 按钮**（GET→405），点击走同一闸门——`DAILY_REPORT_ENABLED` 未开启时
  渲染"未启用"提示且**不调用 LLM、不持久化**；启用后才生成并在正文顶部以结果横幅
  展示标题/总览/要点。仅追加侧栏按钮 + 结果横幅，不改三栏布局。
- ✅ Phase F 第一步（P-004-1）：自定义来源接入**校验 + dry-run 预览**
  `custom_intake.py`（只读）。策略白名单分级（rss/html_index 可选、预留策略可登记不进
  调度、重量策略需 `CUSTOM_SOURCE_ALLOW_RESTRICTED=true`），URL http/https 校验、
  source_key 派生与去重；关键共存结论：`sync_sources_config_to_db` 不删除配置外行，
  自定义来源（DB-only、标记 `user-source`）不被同步清除。设计见
  [V1_CUSTOM_SOURCE_INTAKE_PLAN.md](V1_CUSTOM_SOURCE_INTAKE_PLAN.md)。
- ⏳ Phase F 步骤 2：`add_custom_source` 显式 gate 写库 + `/sources` 添加来源表单（小调整）+ 隔离写入验收。
- ⏳ Phase E：语音播报（TTS，可关闭）。
